...
public class WmNewsServiceImpl  extends ServiceImpl<WmNewsMapper, WmNews> implements WmNewsService {
    ...
    public ResponseResult submitNews(WmNewsDto dto) {
        ...
        //审核文章（异步审核，即在审核方法上家加上@Async注解）
//        wmNewsAutoScanService.autoScanWmNews(wmNews.getId());
        //将审核文章任务放入延时队列，由延迟队列根据发布的时间决定执行审核的时间
    ...
    public ResponseResult findAll(WmNewsScanPageDto dto) {
        /**
         * 需要有作者字段，而wm_news表没有作者字段，所以需要多表查询
         //参数校验：
         dto.checkParam();
    ...
    public ResponseResult updateAuth(WmNewsAdminAuthDto dto, short status) {
        ...
        if (status == WmNews.Status.FAIL.getCode() && StringUtils.isNotBlank(dto.getMsg())) {
            wmNews.setReason(dto.getMsg());
        } else if (status == WmNews.Status.ADMIN_SUCCESS.getCode()) {
